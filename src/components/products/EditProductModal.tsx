...
interface IFormInput {
  ...
  categoryId: string;
  images: string;
  slug: string; // Added slug to form input
}
...
const EditProductModal = ({ product, onClose }: { product: Product; onClose: () => void }) => {
  ...
  useEffect(() => {
    if (product) {
      reset({
        ...
        slug: product.slug, // Set initial slug
      });
    }
  }, [product, reset]);
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data) => {
    ...
    try {
      ...
      onClose();
      // If slug changed, redirect to new product details page
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md animate-fade-in-fast">
        <h2 className="text-2xl font-bold text-rich-black mb-6">Edit Product</h2>
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <InputField label="Slug" error={errors.slug?.message}>
            <input type="text" {...register('slug', { required: 'Slug is required' })} className="w-full p-2 border rounded-md" />
          </InputField>
          ...
          <div className="flex justify-end gap-4 mt-6">
            ...
            <button type="submit" disabled={isLoading} className="px-4 py-2 rounded-md text-white bg-rich-black hover:bg-hooker-green disabled:bg-gray-400 flex items-center gap-2">
              {isLoading && <Spinner size="small" />} {isLoading ? 'Updating...' : 'Update Product'}
            </button>
          </div>
        </form>
      </div>
